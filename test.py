...
#all http filter
cmd_http = 'tshark -r ' + str(file_name) + ' -Y "http" | sort -n > HTTPtraffic.log' + current
os.system(cmd_http)
#count number of packets in the pcap
cnt_pcap = 'tshark -r ' + str(file_name) + ' | wc -l > http.count' + current
os.system(cnt_pcap)
...
cmd_httpcnt = 'tshark -r ' + str(file_name) + ' -Y http | wc -l >> http.count' + current
...
def main():
    ...
    file_name = ('/home/kali/finalproject/http.count' + current)
    with open(file_name) as file_name:
        #call function
        percent(file_name)
...
cmd_mv_filters = 'mv HTTP* ARP* ICMP* PORT* SSH* SYN* UDP* XMAS* -t ~/finalproject/search_filters' + current
